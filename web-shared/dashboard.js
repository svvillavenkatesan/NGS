...
function wireActions() {
  ...
  document.querySelector('#direct-seller-form')?.addEventListener('submit', async (event) => {
    ...
    try {
      if (values.sellerId) {
        await request('/api/users/seller-settings', { method: 'PUT', body: JSON.stringify({ sellerId: values.sellerId, lotCodeId: values.lotCodeId, catalogSchemeRates, commissionPercentage: Number(values.commissionPercentage), actionPassword: values.actionPassword }) });
        notify('Seller Lot Code and Scheme access updated');
      } else {
        await request('/api/users', { method: 'POST', body: JSON.stringify({ ...values, catalogSchemeRates }) });
