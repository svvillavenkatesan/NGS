  ...
  return `<section class="workspace hidden" data-panel="distributors">
    <article class="card wide"><p class="muted">DISTRIBUTOR MANAGEMENT</p><h2>Add Distributor</h2><p class="muted">Add a new distributor or select an existing distributor to update their schemes.</p><form id="distributor-form">
      ...
      <fieldset class="scheme-rates"><legend>3D Common Selection</legend>
        <div class="scheme-toolbar"><p class="muted">இந்த Lot Code-க்கு தேவையான 3D Schemes-ஐ இங்கிருந்து தேர்வு செய்யவும்.</p><button type="button" class="secondary" id="select-all-distributor-schemes">Select All</button></div>
...
function weeklyAccountsPanel(accounts = {}) {
  ...
  return `<section class="workspace hidden" data-panel="weekly-accounts" id="weekly-accounts-panel">
    <article class="card wide">
      <div class="title-row compact"><div><p class="muted">வாரக் கணக்கு · MONDAY TO SUNDAY</p><h2>Weekly Accounts</h2><p class="muted">${escapeHtml(accounts.weekStart || '—')} முதல் ${escapeHtml(accounts.weekEnd || '—')} வரை</p></div><form id="weekly-range-form"><label>பார்க்க வேண்டிய வாரத்தின் தேதி<input name="weekStart" type="date" value="${escapeHtml(accounts.weekStart || '')}" required></label><button type="submit" class="secondary">வாரத்தைப் பார்க்க</button></form></div>
      <h3>1. இந்த வார நிகரக் கணக்கு</h3><p class="muted">மொத்த விற்பனை − பரிசு − மற்ற செலவுகள் = வார இறுதி நிகரத் தொகை</p>
      <div class="metrics"><div><span>மொத்த விற்பனை</span><strong>${money(accounts.totalSales)}</strong></div><div><span>கொடுத்த பரிசு</span><strong>${money(accounts.totalPrizes)}</strong></div><div><span>மற்ற செலவுகள்</span><strong>${money(accounts.totalExpenses)}</strong></div><div><span>வார நிகரத் தொகை</span><strong class="${accounts.finalNetAmount < 0 ? 'error' : ''}">${money(accounts.finalNetAmount)}</strong></div></div>
      <h3>2. ஒவ்வொரு நாளின் இறுதித் தொகை</h3><p class="muted">இந்த அட்டவணையில் ஒவ்வொரு தேதியின் உண்மையான மீதித் தொகை மட்டும் தெளிவாகத் தெரியும்.</p>
      <table><thead><tr><th>தேதி</th><th>Tickets</th><th>விற்பனை</th><th>பரிசு</th><th>செலவு</th><th>அன்றைய இறுதித் தொகை</th></tr></thead><tbody>${days.map((day) => `<tr><td>${escapeHtml(day.date)}</td><td>${day.quantity}</td><td>${money(day.sales)}</td><td>${money(day.prizes)}</td><td>${money(day.expenses)}</td><td class="${day.netAmount < 0 ? 'error' : ''}"><strong>${money(day.netAmount)}</strong></td></tr>`).join('')}</tbody></table>
      <details><summary>அன்றைய செலவை பதிவு செய்ய</summary><form id="daily-expense-form" class="catalog-form"><label>தேதி<input name="expenseDate" type="date" min="${escapeHtml(accounts.weekStart || '')}" max="${escapeHtml(accounts.weekEnd || '')}" value="${escapeHtml(accounts.weekStart || '')}" required></label><label>செலவுத் தொகை<input name="amount" type="number" min="0.01" step="0.01" required></label><label>செலவு விவரம்<input name="note" placeholder="எதற்கான செலவு?" required></label><label>Management Password<input name="actionPassword" type="password" required></label><button>செலவைப் பதிவு செய்</button></form></details>
    </article>
    <article class="card wide"><h3>3. Distributor / Direct Seller வசூல் நிலை</h3><p class="muted">Distributor மற்றும் Super Admin-ன் நேரடி Seller கணக்குகள் தனித்தனியாகக் காட்டப்படும்.</p><div class="metrics"><div><span>செலுத்த வேண்டியது</span><strong>${money(accounts.totalDue)}</strong></div><div><span>இதுவரை வந்த பணம்</span><strong>${money(accounts.totalReceived)}</strong></div><div><span>இன்னும் வர வேண்டியது</span><strong class="${accounts.totalBalance > 0 ? 'error' : ''}">${money(accounts.totalBalance)}</strong></div></div>${rows.length ? `<table><thead><tr><th>Account</th><th>Type</th><th>Tickets</th><th>செலுத்த வேண்டியது</th><th>வந்த பணம்</th><th>இன்னும் வர வேண்டியது</th><th>பணம் பதிவு</th></tr></thead><tbody>${rows.map((row) => `<tr><td>${escapeHtml(row.name)}</td><td>${row.role === 'SELLER' ? 'Direct Seller' : 'Distributor'}</td><td>${row.quantity}</td><td>${money(row.netDue)}</td><td>${money(row.received)}</td><td class="${row.balance > 0 ? 'error' : ''}"><strong>${money(row.balance)}</strong></td><td><form class="weekly-payment-form"><input type="hidden" name="distributorId" value="${escapeHtml(row.distributorId)}"><input type="hidden" name="weekStart" value="${escapeHtml(accounts.weekStart)}"><input name="amount" type="number" min="0.01" step="0.01" placeholder="வந்த தொகை" required><input name="reference" placeholder="Reference / Note"><input name="actionPassword" type="password" placeholder="Management Password" required><button>பணம் வந்தது</button></form></td></tr>`).join('')}</tbody></table>` : '<p class="muted">கணக்குகள் இல்லை.</p>'}</article>
  </section>`;
}
...
function reportDetailHtml(report) {
  ...
  return `<article class="card wide report-sheet"><div class="report-header"><div><p class="muted">PERMANENT SALE REPORT</p><h2>${escapeHtml(report.reportId)}</h2></div><button type="button" class="print-report" data-report-id="${escapeHtml(report.id)}">Print A4</button></div><div class="report-meta"><div><span>Seller</span><strong>${escapeHtml(report.seller.name)} / ${escapeHtml(report.seller.id)}</strong></div><div><span>Distributor</span><strong>${escapeHtml(report.distributor.name)}</strong></div><div><span>Lot Code / Show</span><strong>${escapeHtml(report.boardCode)} / ${escapeHtml(report.showLabel)}</strong></div><div><span>Report Date</span><strong>${escapeHtml(report.businessDate)}</strong></div><div><span>Created</span><strong>${escapeHtml(new Date(report.createdAt).toLocaleString('en-IN'))}</strong></div><div><span>Status</span><strong>${escapeHtml(report.status)}</strong></div></div><div class="bill-table"><table class="report-entry-table"><thead><tr><th>Sl.</th><th>Time</th><th>Scheme</th><th>Entered Number</th><th>Qty</th><th>Winning Number</th><th>Sale</th><th>Prize</th>${canCorrect ? '<th>Super Admin Correction</th>' : ''}</tr></thead><tbody>${report.entries.map((entry) => `<tr><td>${entry.sequence}</td><td>${escapeHtml(entry.time)}</td><td>${escapeHtml(entry.scheme)}</td><td class="number compact-number">${escapeHtml(entry.enteredNumber)}</td><td>${entry.quantity}</td><td class="number compact-number">${escapeHtml(entry.winningNumber ?? '----')}</td><td>${money(entry.saleAmount)}</td><td>${money(entry.prizeAmount)}${entry.corrected ? '<br><span class="status break_even">CORRECTED</span>' : ''}</td>${canCorrect ? `<td><form class="report-correction-form"><input type="hidden" name="reportId" value="${escapeHtml(report.id)}"><input type="hidden" name="transactionId" value="${escapeHtml(entry.transactionId)}"><input name="enteredNumber" inputmode="numeric" value="${escapeHtml(entry.enteredNumber)}" aria-label="Correct number" required><input name="quantity" type="number" min="1" max="1000" value="${entry.quantity}" aria-label="Correct quantity" required><input name="reason" placeholder="Correction reason" minlength="5" required><input name="actionPassword" type="password" placeholder="Management Password" required><button>Correct</button></form></td>` : ''}</tr>`).join('')}</tbody><tfoot><tr><th colspan="4">TOTAL</th><th>${report.totalQuantity}</th><th></th><th>${money(report.totalSales)}</th><th>${money(report.totalPrize)}</th>${canCorrect ? '<th></th>' : ''}</tr></tfoot></table></div>${auditRows}</article>`;
}
...
function printableReportHtml(report) {
  const rows = report.entries.map((entry) => `<tr><td>${entry.sequence}</td><td>${escapeHtml(entry.time)}</td><td>${escapeHtml(entry.scheme)}</td><td>${escapeHtml(entry.enteredNumber)}</td><td>${entry.quantity}</td><td>${escapeHtml(entry.winningNumber ?? '----')}</td><td>${money(entry.saleAmount)}</td><td>${money(entry.prizeAmount)}</td></tr>`).join('');
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(report.reportId)}</title><style>@page{size:A4 landscape;margin:12mm}body{font-family:Arial,sans-serif;color:#111;font-size:11px}h1{font-size:20px;margin:0 0 8px}.meta{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin:12px 0}.meta div{border:1px solid #bbb;padding:7px}.meta span{display:block;color:#555;font-size:9px}table{width:100%;border-collapse:collapse}thead{display:table-header-group}th,td{border:1px solid #999;padding:6px;text-align:left}tfoot{font-weight:bold;background:#eee}.lock{float:right;border:1px solid #111;padding:5px}</style></head><body><span class="lock">PERMANENT / DELETE LOCKED</span><h1>${escapeHtml(report.reportId)}</h1><div class="meta"><div><span>Seller / ID</span>${escapeHtml(report.seller.name)} / ${escapeHtml(report.seller.id)}</div><div><span>Distributor</span>${escapeHtml(report.distributor.name)}</div><div><span>Lot Code / Show</span>${escapeHtml(report.boardCode)} / ${escapeHtml(report.showLabel)}</div><div><span>Report Date</span>${escapeHtml(report.businessDate)}</div><div><span>Created</span>${escapeHtml(new Date(report.createdAt).toLocaleString('en-IN'))}</div><div><span>Status / Winning Number</span>${escapeHtml(report.status)} / ${escapeHtml(report.winningNumber ?? '----')}</div></div><table><thead><tr><th>Sl.</th><th>Time</th><th>Scheme</th><th>Entered Number</th><th>Qty</th><th>Winning Number</th><th>Sale</th><th>Prize</th></tr></thead><tbody>${rows}</tbody><tfoot><tr><td colspan="4">TOTAL</td><td>${report.totalQuantity}</td><td></td><td>${money(report.totalSales)}</td><td>${money(report.totalPrize)}</td></tr></tfoot></table><script>window.onload=()=>window.print()<\/script></body></html>`;
}
function wireReportActions() {
  ...
  document.querySelectorAll('.print-report').forEach((button) => button.addEventListener('click', async () => { const printWindow = window.open('', '_blank'); try { const report = await request(`/api/reports/sale?reportId=${encodeURIComponent(button.dataset.reportId)}`); printWindow.document.write(printableReportHtml(report)); printWindow.document.close(); } catch (error) { printWindow?.close(); notify(error.message, true); } }));
...
function wireActions() {
  ...
  document.querySelector('#direct-seller-form')?.addEventListener('submit', async (event) => {
    ...
    try { await request('/api/users', { method: 'POST', body: JSON.stringify({ ...values, catalogSchemeRates }) }); notify('Direct Seller created under Super Admin'); await renderDashboard(); switchPanel('direct-sellers'); }
    catch (error) { notify(error.message, true); }
  });
  ...
  document.querySelector('#direct-seller-lot-code')?.addEventListener('change', updateDirectSellerSchemes);
