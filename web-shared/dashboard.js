...
function reportDetailHtml(report) {
  ...
  return `<article class="card wide report-sheet"><div class="report-header"><div><p class="muted">DETAILED SALE REPORT</p><h2>${escapeHtml(report.reportId)}</h2></div><button type="button" class="print-report" data-report-id="${escapeHtml(report.id)}">Print A4 Landscape</button></div><div class="report-meta"><div><span>Seller</span><strong>${escapeHtml(report.seller.name)} / ${escapeHtml(report.seller.id)}</strong></div><div><span>Distributor</span><strong>${escapeHtml(report.distributor.name)}</strong></div><div><span>Lot Code / Show</span><strong>${escapeHtml(report.boardCode)} / ${escapeHtml(report.showLabel)}</strong></div><div><span>Report Date</span><strong>${escapeHtml(report.businessDate)}</strong></div><div><span>Result</span><strong>${escapeHtml(report.winningNumber ?? 'Not published')}</strong></div><div><span>Status</span><strong>${escapeHtml(report.status)}</strong></div></div><div class="bill-table"><table class="report-entry-table detailed-report-table"><thead><tr><th>Sl.</th><th>Date</th><th>Bill No.</th><th>Time</th><th>Lot</th><th>Show</th><th>Scheme</th><th>Number</th><th>Qty</th><th>Rate</th><th>Sold</th><th>Prize</th><th>Prize reason</th><th>Bonus</th><th>Net</th>${canCorrect ? '<th>Super Admin Correction</th>' : ''}</tr></thead><tbody>${rows}</tbody><tfoot><tr><th colspan="8">TOTAL</th><th>${report.totalQuantity}</th><th></th><th>${money(report.totalSales)}</th><th>${money(report.totalPrize)}</th><th></th><th>${money(report.totalBonus)}</th><th>${money(report.totalNet)}</th>${canCorrect ? '<th></th>' : ''}</tr></tfoot></table></div><p class="muted">Net = Sold − Bonus − Prize. Negative Net means the Seller has money to receive.</p>${auditRows}</article>`;
}
...
function printableReportHtml(report) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(report.reportId)}</title><style>@page{size:A4 landscape;margin:8mm}body{font-family:Arial,sans-serif;color:#111;font-size:8px}h1{font-size:17px;margin:0 0 6px}.meta{display:grid;grid-template-columns:repeat(5,1fr);gap:4px;margin:8px 0}.meta div{border:1px solid #bbb;padding:5px}.meta span{display:block;color:#555;font-size:7px}table{width:100%;border-collapse:collapse;table-layout:auto}thead{display:table-header-group}th,td{border:1px solid #999;padding:3px;text-align:left;white-space:nowrap}tfoot{font-weight:bold;background:#eee}.lock{float:right;border:1px solid #111;padding:4px}.note{margin-top:6px}</style></head><body><span class="lock">PERMANENT / DELETE LOCKED</span><h1>DETAILED SALE REPORT · ${escapeHtml(report.reportId)}</h1><div class="meta"><div><span>Seller / ID</span>${escapeHtml(report.seller.name)} / ${escapeHtml(report.seller.id)}</div><div><span>Distributor</span>${escapeHtml(report.distributor.name)}</div><div><span>Lot / Show</span>${escapeHtml(report.boardCode)} / ${escapeHtml(report.showLabel)}</div><div><span>Date</span>${escapeHtml(report.businessDate)}</div><div><span>Status / Result</span>${escapeHtml(report.status)} / ${escapeHtml(report.winningNumber ?? 'Not published')}</div></div><table><thead><tr><th>Sl.</th><th>Date</th><th>Bill No.</th><th>Time</th><th>Lot</th><th>Show</th><th>Scheme</th><th>Number</th><th>Qty</th><th>Rate</th><th>Sold</th><th>Prize</th><th>Prize reason</th><th>Bonus</th><th>Net</th></tr></thead><tbody>${rows}</tbody><tfoot><tr><td colspan="8">TOTAL</td><td>${report.totalQuantity}</td><td></td><td>${money(report.totalSales)}</td><td>${money(report.totalPrize)}</td><td></td><td>${money(report.totalBonus)}</td><td>${money(report.totalNet)}</td></tr></tfoot></table><div class="note">Net = Sold − Bonus − Prize. Negative Net means amount receivable by Seller.</div><script>window.onload=()=>window.print()<\/script></body></html>`;
}
